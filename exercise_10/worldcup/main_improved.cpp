...
void testcase() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        lp.set_b(3 * i, d);
        lp.set_b(3 * i + 1, -d);
        // We need to convert "u" into a percentage value, as the read values "a" are percentages.
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            int r; std::cin >> r;
            // We work with Rappen, so we can avoid floatingpoint values.
            revenue[i][j] = r * 100;
        }
    }
    ...
    if (s.is_infeasible()) {
        ...
        // Convert the Rappen back to swiss francs.
        std::cout << std::setprecision(0) << std::fixed << std::floor(floor_to_double(-s.objective_value()) / 100) << std::endl;
    }
}
