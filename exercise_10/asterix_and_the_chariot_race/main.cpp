...
int solve(int position, bool on, bool parent) {
    ...
    if (on) {
        ...
        // In this case we need at least one neighbor to repair their streets.
        // But there can also be more. We solve this by calculating the overall
        // minimum and the minimum for each neighbor. As such, it could be the
        // case, that all neighbors would generate a minimum when they don't
        // repair their streets. To fix this problem, we compute the best option
        // where we require that at least one neighbor repairs their street. If
        // the total minimum contains a neighbor that repairs its streets, then
        // our final best option will be the same as the total minimum itself.
