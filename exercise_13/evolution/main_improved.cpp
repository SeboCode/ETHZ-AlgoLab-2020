...
void dfs(ages_type const & ages, children_type const & children, queries_type const & queries, results_type & results, stack_type & stack, int currentSpeciesIndex) {
    ...
    for (std::pair<int, int> query : queries[currentSpeciesIndex]) {
        // std::lower_bound: Returns the first element that no longer satisfies the condition.
        // Requires that all elements for which the expression is true must precede all elements
        // for which the expression is false. It can thus use a binary search internally. 
        results[query.second] = (*std::lower_bound(stack.begin(), stack.end(), query.first, [] (auto const & element, const int value) {
